  ...
  try {
    ...
    if (transporter) {
      const mailOptions = {
        ...
        html: `
          <div style="font-family: Arial, sans-serif; padding: 20px; color: #333;">
            <h2>Reset your PizzaCraft password 🍕</h2>
            <p>We received a request to reset your password.</p>
            <p>Click the button below to create a new password:</p>
            <a href="${resetUrl}" style="display:inline-block; padding:12px 24px; background:#f97316; color:white; text-decoration:none; border-radius:12px; font-weight:bold;">
              Reset Password
            </a>
            <p style="margin-top: 20px; font-size: 12px; color: #777;">Or copy this link: <a href="${resetUrl}">${resetUrl}</a></p>
            <p style="font-size: 12px; color: #777;">This link expires in 15 minutes. If you didn't request this, you can safely ignore this email.</p>
          </div>
        `,
      };
      await transporter.sendMail(mailOptions);
    ...
    console.error('SMTP Email sending error (reset link printed in console above):', err.message);
  }
};
